...
private:
    ...
    std::unordered_map<std::string, llvm::AllocaInst*> autoAllocas;
...
inline std::unique_ptr<llvm::Module> createModule(llvm::LLVMContext& ctx, const std::string& name, const AbstractSyntaxTree& ast) {
    ...
    for (const std::pair<std::string, int>& externFunc : ast.getExternFunctions()) {
        llvm::FunctionType* externFuncType = llvm::FunctionType::get(
            ty, std::vector<llvm::Type*>(externFunc.second, ty), false);

        llvm::Function::Create(
            externFuncType, llvm::Function::ExternalLinkage, externFunc.first, *module);
    }
